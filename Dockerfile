...
RUN cargo build --release -p w9-links-creator-server 2>/dev/null || true
COPY server/src ./server/src
RUN cargo build --release -p w9-links-creator-server &&     cp target/release/w9-links-creator-server /usr/local/bin/appserver &&     chmod +x /usr/local/bin/appserver &&     ls -la /usr/local/bin/appserver
