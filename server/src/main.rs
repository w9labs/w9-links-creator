...
fn pub_layout(t: &str, b: &str) -> String {
    layout(t, b, r#"<a href="/login">Login</a>"#)
}
fn user_layout(t: &str, b: &str) -> String {
    layout(
        t,
        b,
        r#"<a href="/links">Links</a><a href="/notes">Notes</a><a href="/logout">Logout</a>"#,
    )
}

fn set_s(j: CookieJar, t: String) -> CookieJar {
    j.add(
        axum_extra::extract::cookie::Cookie::build(("w9_link_session", t))
            .path("/")
            .http_only(true)
...
fn clr_s(j: CookieJar) -> CookieJar {
    j.remove(axum_extra::extract::cookie::Cookie::named(
        "w9_link_session",
    ))
}
fn get_s(j: &CookieJar) -> Option<String> {
    j.get("w9_link_session").map(|c| c.value().to_string())
}
...
fn home_html() -> String {
    pub_layout(
        "W9 Links",
        r#"<div class="hero"><img class="hero-logo" src="/w9-logo/logo-landscape-transparent.svg" alt="W9 Labs"/><h1>W9 Links</h1><p class="hero-sub">Short links on w9.nu / w9.se + private note drops</p><p class="hero-muted">Create, share, and track your links</p><div class="hero-actions"><a href="https://db.w9.nu/oauth/authorize?redirect_uri=https://links.w9.nu/oauth/callback&response_type=code&client_id=w9-links" class="btn" onclick="const w=window.open(this.href,'w9-links-login','width=520,height=720'); if (w) { w.focus(); return false; }">Login with W9</a></div></div><div class="grid"><div class="card"><h3>📎 Short Links</h3><p>Create branded short URLs with click tracking.</p></div><div class="card"><h3>📝 Note Drops</h3><p>Password-protected notes that auto-destroy.</p></div></div>"#,
    )
}
fn login_html() -> String {
    pub_layout(
        "Login",
        r#"<div class="card" style="max-width:420px;margin:3rem auto;text-align:center"><h1>🔗 W9 Links</h1><p class="text-sm text-muted mb-2">Sign in with W9 DB</p><a href="https://db.w9.nu/oauth/authorize?redirect_uri=https://links.w9.nu/oauth/callback&response_type=code&client_id=w9-links" class="btn" style="width:100%" onclick="const w=window.open(this.href,'w9-links-login','width=520,height=720'); if (w) { w.focus(); return false; }">Login with W9 DB</a></div>"#,
    )
}

fn popup_close_html(target: &str) -> String {
    format!(
        r#"<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>W9 Links Login</title></head><body><script>(function(){{const target = {target:?}; if (window.opener && !window.opener.closed) {{ try {{ window.opener.location.href = target; window.opener.focus(); }} catch (_) {{}} window.close(); }} else {{ window.location.replace(target); }}}})();</script><p>Signing you in…</p></body></html>"#
    )
}
...
fn note_view_html(c: &str, ct: &str) -> String {
    user_layout(
        "View Note",
        &format!(
            r#"<div class="card" style="max-width:700px;margin:2rem auto"><h1>📝 Note: {}</h1><div class="code">{}</div><a href="/notes" class="btn mt-2">Back</a></div>"#,
            c, ct
        ),
    )
}
...
async fn oauth_cb(
    ...
    let res = match s
        ...
        .form(&[
            ...
            ("code", &code),
            ("redirect_uri", "https://links.w9.nu/oauth/callback"),
        ])
        .send()
        .await
    {
    ...
    (set_s(jar, token), Html(popup_close_html("/links"))).into_response()
}
...
async fn notes_post(
    ...
    let pw = f.password.as_ref().map(|p| {
...
async fn view_note(
    ...
    if let Some(m) = mx {
        if views >= m {
            let _ =
                s.db.execute("DELETE FROM notes WHERE code=$1", &[&code])
                    .await;
    ...
    let _ =
        s.db.execute("UPDATE notes SET views=views+1 WHERE code=$1", &[&code])
            .await;
...
async fn health(State(s): State<AppState>) -> impl IntoResponse {
    match s.db.query_one("SELECT 1", &[]).await {
        Ok(_) => (
            StatusCode::OK,
            Json(
                serde_json::json!({"status":"ok","service":"w9-links-creator","database":"connected","timestamp":Utc::now().to_rfc3339()}),
            ),
        ),
        Err(e) => (
